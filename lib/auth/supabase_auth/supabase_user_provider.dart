...
class SafeBoxSupabaseUser extends BaseAuthUser {
  SafeBoxSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> safeBoxSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SafeBoxSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
